...
import org.tensorflow.demo.DetectionInfo;
...
public class MultiBoxTracker {
  ...
  public synchronized void drawDebug(final Canvas canvas) {
    ...
    for (final TrackedRecognition recognition : trackedObjects) {
      ...
      if (getFrameToCanvasMatrix().mapRect(trackedPos)) {
        ...
        logger.i("2222gridcam===>canvas.drawText labelString = " + labelString);
  ...
  public synchronized DetectionInfo draw(final Canvas canvas) {
    DetectionInfo detectionInfo = new DetectionInfo(0f,0f);
    ...
    for (final TrackedRecognition recognition : trackedObjects) {
      ...
      logger.i("gridcam===>11111canvas.drawText mPixelPerDist = " + ImageUtils.mPixelPerDist);
      if (ImageUtils.mPixelPerDist > 0) {
        float hMulti = canvas.getHeight() / (float) (rotated ? frameWidth : frameHeight);
        float wMulti = canvas.getWidth() / (float) (rotated ? frameHeight : frameWidth);
        float vHeight = (trackedPos.right - trackedPos.left) * 1.0f / (ImageUtils.mPixelPerDist * hMulti * wMulti);
        float hLength = (trackedPos.bottom - trackedPos.top) * 1.0f / (ImageUtils.mPixelPerDist * hMulti * wMulti);
        logger.i("gridcam===>trackedPos = " + trackedPos.toString());
        labelString = String.format("w:%.2f,h:%.2f", vHeight, hLength);
        detectionInfo.setvHeight((float)(Math.round(vHeight*100))/100);
        detectionInfo.sethLength((float)(Math.round(hLength*100))/100);
      } else {
        labelString = "";
      }
      logger.i("gridcam===>11111canvas.drawText labelString = " + labelString);
      borderedText.drawText(canvas, trackedPos.left + cornerSize, trackedPos.bottom, labelString);
      //save vHeight & hLength
    }
    logger.i("test_info activity vHeight = " + detectionInfo.getvHeight());
    logger.i("test_info activity hLength = " + detectionInfo.gethLength());
