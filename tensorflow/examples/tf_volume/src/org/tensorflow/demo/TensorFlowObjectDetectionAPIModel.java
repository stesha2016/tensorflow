...
public class TensorFlowObjectDetectionAPIModel implements Classifier {
  ...
  public List<Recognition> recognizeImage(final Bitmap bitmap) {
    ...
    for (int i = 0; i < outputScores.length; ++i) {
      ...
      int pixelPerDist = 0;
      if (outputScores[i] > 0.6f) {
        LOGGER.i("gridcam===>outputClasses[i] = " + outputClasses[i]);
        LOGGER.i("gridcam===>outputScores[i] = " + outputScores[i]);
        LOGGER.i("gridcam===>detection = " + detection.toString());
        pixelPerDist = ImageUtils.getDistancePixel();
        LOGGER.i("gridcam===>pixelPerDist = " + pixelPerDist);
      }
      pq.add(
          new Recognition("" + i, labels.get((int) outputClasses[i]), outputScores[i], detection, pixelPerDist));
    }
